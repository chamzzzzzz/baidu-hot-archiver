...
func archive() {
	...
	os.MkdirAll("archives/baidu", 0755)
	name := fmt.Sprintf("archives/baidu/%s.txt", time.Now().Format("2006-01-02"))
	...
	n := 0
	for _, div := range dom.QueryAll("div", "class", "c-single-text-ellipsis") {
		word := div.Text()
